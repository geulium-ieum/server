...
public class GuestbookService {
    ...
    public void update(Long entryId, UserInfo user, GuestbookRequest request) {
        ...
        if (!user.getId().equals(e.getUserId())) throw new ApiException(ErrorCode.FORBIDDEN);
        if (request.getAuthorName() != null) e.setAuthorName(request.getAuthorName());
        if (request.getContent() != null) e.setContent(request.getContent());
